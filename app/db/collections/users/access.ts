...
export const isStaff: Access<User> = ({ req: { user } }) => {
  return Boolean(user?.roles?.includes("staff"));
};

// This work because the document id here is the user being accessed
export const isSelf: Access<User> = ({ req: { user }, id }) => {
  return user?.id === id;
};

export const isStaffOrSelf: Access<User> = (props) =>
...
export const isSelfFieldLevel: FieldAccess = ({ req, doc }) => {
  return req.user?.id === doc?.id;
};

export const isStaffOrSelfFieldLevel: FieldAccess = (props) =>
